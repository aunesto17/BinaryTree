...
public:
    ...
    CtreeIterator<Tr> operator ++ (int){
        par * t = &m_p.top();
        //Mientras que no tengamos el siguiente elemento a mostrar(diferente de 1)
        while (t->second!=1) {
            switch (t->second) {
...
public:
    ...
    void printTreeRecursive(cNode<T>*,T,T);
    int maxTreeHeight(cNode<T>*);
...
template<class Tr>
int CbinTree<Tr>::maxTreeHeight(cNode<T> * r)
{
    //caso base
    if (r == nullptr) {
        return 0;
        ...
        int lDepth = maxTreeHeight(r->m_child[0]);
        int rDepth = maxTreeHeight(r->m_child[1]);
...
//recursive print
template<class Tr>
void CbinTree<Tr>::printTreeRecursive(cNode<T> * r,T h,T l)
{
    //imprimimos root
    if (l==0) {
        cout << setw(10*h) << r->m_data << endl;
        for (int i=1; i<=h; ++i) {
            cout << setw(10*h-i-1) << "/" << setw(2*i+1) << "\\";
            cout << endl;
        }
    }
    //si es hijo
    else if (r!=nullptr){
        cout << setw(10*h+6) << r->m_data;
    }
    l++;
    if (r != nullptr && l!=0) {
        printTreeRecursive(r->m_child[0],h-1,l);
        printTreeRecursive(r->m_child[1],h-2,l);
    }
